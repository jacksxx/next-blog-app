        ...
        <div className='px-5 mx-5 border-b-[1px] border-black'>
            <div className='flex items-center justify-between py-4 relative'>
                ...
                <div className='flex items-center space-x-4'>
                    <SearchBar />
                    <div onClick={() => setShowProfile(!showProfile)} className='relativa cursor-pointer'>
                        <Image src={usericon} width={35} height={35} className='rounded-full object-cover' alt='' />
                        <SignOut showProfile={showProfile} />
                    </div>
                    <span onClick={() => setShowNav(!showNav)} className={`p-[9px] bg-gray-200 rounded-full md:hidden transition ease-in duration-150 ${showNav ? "rotate-180" : "0"}`}><BsChevronCompactUp /></span>
                </div>
            </div>
